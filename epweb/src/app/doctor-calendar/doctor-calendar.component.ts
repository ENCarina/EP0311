...
export class DoctorCalendarComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.authService.getRoleId() !== 1) {
      this.router.navigate(['/sajat-foglalasaim']);
      return;
    }
    ...
    this.bookingApi.getMyBookings().subscribe({
      next: (bookings: any[]) => {
        this.allBookings = (bookings || [])
          .map((booking) => this.mapBookingToCalendarItem(booking))
          .filter((booking): booking is CalendarBookingItem => booking !== null);
  ...
  private mapBookingToCalendarItem(booking: any): CalendarBookingItem | null {
    const date = booking?.timeSlot?.date || booking?.date;
    const startTime = booking?.timeSlot?.startTime || booking?.startTime;
    const rawEndTime = booking?.timeSlot?.endTime || booking?.endTime;
    ...
    return {
      ...
      consultationName: booking?.treatment?.name || booking?.name || 'Vizsgálat',
      doctorName: booking?.doctor?.user?.name || this.authService.getUserName(),
