...
export const EmailService = {
    async sendWelcomeEmail(userEmail, userName, verifyUrl, lang = 'hu') {
        const subjects = {
            hu: 'Üdvözlünk az ElitPort rendszerében!',
            en: 'Welcome to the ElitPort system!'
        };
            ...
            const info = await transporter.sendMail({
                ...
                <div style="font-family: Arial, sans-serif; color: ${COLORS.text}; max-width: 600px; margin: auto; border: 1px solid ${COLORS.silver}; background-color: ${COLORS.white}; padding: 20px;"> 
                    <div style="text-align: center; border-bottom: 3px solid ${COLORS.darkBlue}; padding-bottom: 20px; margin-bottom: 20px;">
                        <h1 style="color: ${COLORS.darkBlue}; margin: 0; font-size: 24px;">
                            ${lang === 'hu' ? 'Üdvözlünk!' : 'Welcome!'}
                        </h1>
                    </div>
                    ...
                    <p style="font-size: 16px; line-height: 1.5;">
                        ...
                        ? 'Köszönjük, hogy regisztráltál az <strong>Elit Klinika</strong> online rendszerébe. Már csak egy lépés választ el a teljes hozzáféréstől.' 
                            : 'Thank you for registering at <strong>Elit Clinic</strong> online system. You are just one step away from full access.'} 
                        </p>
                        <div style="text-align: center; margin: 30px 0;">
                            <a href="${verifyUrl}" style="background: #0d6efd; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px; font-weight: bold;">
                            ${lang === 'hu' ? 'Email cím megerősítése' : 'Verify Email Address'}
                            </a>
                        </div>
                        
                        <p style="font-size: 14px; color: #666;">
                            ${lang === 'hu' ? 'Ha a gomb nem működik, másold be ezt a linket:' : 'If the button does not work, copy this link:'} ${verifyUrl}
                        </p>
                    
                        <p style="margin-top: 30px; line-height: 1.5;">${lang === 'hu' ? 'Várjuk szeretettel!' : 'Best regards,'}</p>
                    
                        <hr style="border: 0; border-top: 1px solid ${COLORS.silver}; margin-top: 40px; margin-bottom: 10px;">
                        <p style="font-size: 12px; color: ${COLORS.darkBlue}; text-align: center; margin: 0;">
                            ${lang === 'hu' ? 'Ez egy automatikus üzenet az ElitPort rendszeréből.' : 'This is an automated message from the ElitPort system.'}
                        </p>
                    </div>
                    `
                });
